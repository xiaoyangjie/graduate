...
class Proxy(object):
    ...
    def proxyStatus(self):
        ...
        for i in range(len(PROXYLIST)):
            ########代理格式##################
            proxies = {'http':'http://%s' %(PROXYLIST[i].get('host')+':'+str(PROXYLIST[i].get('port'))),
                        'https':'http://%s' %(PROXYLIST[i].get('host')+':'+str(PROXYLIST[i].get('port')))}
            try:
                r = requests.get(TESTURL,proxies=proxies)  #访问GOOGLE

                if r.status_code == 200:###########说明这个代理是好的######################
                    if PROXYLIST[i] not in self.proxyList:
                        self.proxyList.append(PROXYLIST[i])#############添加代理############
                else:
                    if PROXYLIST[i] in self.proxyList:###########说明这个代理是不能使用的######################
                        self.proxyList.remove(PROXYLIST[i])  ##########删除代理################
            except:pass

        ###########每过30秒重新检测一次，使用的是异步#######################
        threading.Timer(30, self.proxyStatus).start()
...
class PipeThread(threading.Thread):
    ...
    def __init__(self, source, sink, aa):
        self.aa = aa
...
class Pinhole(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.proxyList == []:
                self.proxyList = PROXYLIST   #如果没有可使用的代理，则给出全部代理（主要是怕代理使用过多，而不是真正的没有代理了）
            try:
                ...
                PipeThread(newsock, fwd, 11).start() #正向传送，数据发往代理
                # PipeThread(fwd, newsock, 22).start() #逆向传送，从代理接收数据
            except:pass
